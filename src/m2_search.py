    ...
    def search(self, query: str, top_k: int = BM25_TOP_K) -> list[SearchResult]:
        ...
        for i in top_indices:
            if scores[i] > 0:
                ...
                results.append(SearchResult(text=doc["text"], score=scores[i], metadata=doc.get("metadata", {}), method="bm25"))
        return results
...
class DenseSearch:
    ...
    def index(self, chunks: list[dict], collection: str = COLLECTION_NAME) -> None:
        ...
        for i, (text, vector, chunk) in enumerate(zip(texts, vectors, chunks)):
            points.append(PointStruct(id=i, vector=vector.tolist(), payload={**chunk.get("metadata", {}), "text": text}))
    ...
    def search(self, query: str, top_k: int = DENSE_TOP_K, collection: str = COLLECTION_NAME) -> list[SearchResult]:
        ...
        for pt in response.points:
            results.append(SearchResult(text=pt.payload["text"], score=pt.score, metadata=pt.payload, method="dense"))
        return results
...
def reciprocal_rank_fusion(results_list: list[list[SearchResult]], k: int = 60,
    ...
    for item in top_items:
        res = item["result"]
        final_results.append(SearchResult(text=res.text, score=item["score"], metadata=res.metadata, method="hybrid"))
