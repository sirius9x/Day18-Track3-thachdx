...
def chunk_semantic(text: str, threshold: float = SEMANTIC_THRESHOLD,
    ...
    for i in range(1, len(sentences)):
        ...
        if sim < threshold:
            chunks.append(Chunk(text=" ".join(current_chunk), metadata={**metadata, "strategy": "semantic"}))
...
def chunk_hierarchical(text: str, parent_size: int = HIERARCHICAL_PARENT_SIZE,
    ...
    current_parent = ""
    for p in paragraphs:
        if len(current_parent) + len(p) > parent_size and current_parent:
            pid = f"parent_{len(parents)}"
            parents.append(Chunk(text=current_parent.strip(), metadata={**metadata, "chunk_type": "parent", "parent_id": pid}))
            
            c_paras = [cp.strip() for cp in current_parent.strip().split("\n") if cp.strip()]
            curr_child = ""
            for cp in c_paras:
